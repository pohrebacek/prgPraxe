...
def move(x, check_button, delete_button):
    if (x.get() == 1):
        print("Moved")
        ...
        for i in range(0,len(line)):
            if line[i] == "-":
                ...
                delete_button = Button(window,
                                        ...
                                        image=crossImageFinal)
                delete_button.grid(row=count+2, column=1)
                delete_button.place(relx=0.75, anchor="e", y=200)
...
#entryPlusAdd = LabelFrame(window)
##entryPlusAdd.grid_propagate(0)
#entryPlusAdd.grid(row=0, column=0)
##entryPlusAdd.pack(side=TOP)
...
#add_button.place(x=100)
#add_button.place(relx=0.5, rely=(entry.winfo_height()/10), anchor=CENTER)
