document.addEventListener("DOMContentLoaded", (event) => {
    ...
    sio.on ("createRoom", (data) => {
        ...
        if (data.isAdded) {
            ...
            buttonDelete.onclick = function(){leaveRoom(this)};
        } else {
    ...
    function deleteRoom(del) {
        ...
        roomId = delParent.children[1];
        sio.emit("deleteRoom", roomId.textContent);
        delParent.remove();
    };
    ...
    sio.on("deleteRoomFromMenu", (data) => {
        var rooms = document.getElementById("chatRooms").children;
        for (let i = 0; i < rooms.length; i++){ 
            room = rooms[i].children;
            if (room[1].textContent == data.roomId) {
                rooms[i].remove();
                break;
            }
        };
